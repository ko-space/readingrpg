...
(function () {
    ...
    function onUnitConfigChange(slot) {
        ...
        units[slot] = {
            name, maxHp, hp: maxHp, atk, isMelee, outfit, style,
        ...
        clearWinAura(slot); // 캐릭터/성급이 바뀌면 이전 유닛의 오라(스택)는 의미가 없으니 지운다
        renderUnit(slot);
    }
        ...
        if (isDead) {
            ...
            if (imgEl) {
                ...
                if (!attackAnimActive[slot]) {
                    ...
                    imgEl.classList.toggle("flipped", slot.startsWith("defender"));
                }
            }
        }
    ...
    function setupUnitSelection() {
        ...
        [...SLOTS, "attacker-summon", "defender-summon"].forEach((slot) => {
            ...
            el.addEventListener("click", () => {
                if (!units[slot]) return; // 아직 소환되지 않은 자리는 선택할 수 없음
    ...
    const MANUAL_SKILL_HANDLERS = {
        ...
        summon_clone(casterSlot, params) {
            ...
            if (cloneEl) {
                ...
                cloneEl.style.transform = ""; // 매번 소환될 때마다 자신의 시작 위치로 초기화
            }
        ...
        stun_target(casterSlot, params) {
            ...
            renderUnit(targetSlot);
            return { text: `${units[targetSlot].name} ${params.seconds}초 기절` };
        },
    ...
    function updateWinAura(slot, stackCount) {
        const imgEl = document.querySelector(`[data-unit="${slot}"] .battle-unit-img`);
    ...
    function clearWinAura(slot) {
        const imgEl = document.querySelector(`[data-unit="${slot}"] .battle-unit-img`);
        if (!imgEl) return;
        imgEl.classList.remove("win-aura");
        imgEl.style.removeProperty("--aura-color");
        imgEl.style.removeProperty("--aura-blur");
    }
    ...
    function getGapToTarget(unitKey, targetKey) {
        ...
        return isAttacker ? (targetRect.left - rect.right) + 100 : (targetRect.right - rect.left) - 100;
    }
    ...
    function startMeleeWalker() {
        Object.keys(units).forEach((slot) => {
            ...
            meleeTargetKey[slot] = opponentFrontSlot(slot);
            meleeArrived[slot] = false;
        });
        walkerRunning = true;
        ...
        function tick() {
            ...
            Object.keys(units).forEach((slot) => {
                ...
                const forwardSign = slot.startsWith("attacker") ? 1 : -1;
    ...
    function setupManualButtons() {
        ...
        document.getElementById("dt-use-skill").addEventListener("click", () => {
            ...
            setTimeout(() => {
                ...
                const unitEl = document.querySelector(`[data-unit="${actorSlot}"]`);
                unitEl?.classList.add(`skill-vfx-${category}`);
                setTimeout(() => unitEl?.classList.remove(`skill-vfx-${category}`), 450);

                // 캐릭터 전용 스킬 발사체 연출 - "전투 게시"(서버 재생)와 같은 함수를 그대로 재사용한다.
                // 수동 모드는 즉석 확인용이라 피해는 이미 위 handler에서 즉시 반영됐고, 투사체는 눈으로
                // 보여주는 용도로만 곁들인다(도착 시점에 맞춰 피해를 늦추지 않음).
                if (dispatchEffectType === "self_stack_buff") {
                    updateWinAura(actorSlot, units[actorSlot].status.stackCount);
                } else if (dispatchEffectType === "self_shield_duration") {
                    spawnShieldRing(actorSlot, params.seconds * 1000);
                } else if (dispatchEffectType === "conditional_target_debuff" && result.stunned && result.targetSlot) {
                    playDualCrayonSkillProjectile(actorSlot, result.targetSlot, () => {
                        const targetUnitEl = document.querySelector(`[data-unit="${result.targetSlot}"]`);
                    ...
                    spawnHealingHeart(result.targetSlot, () => {
                        const healImgEl = document.querySelector(`[data-unit="${result.targetSlot}"] .battle-unit-img`);
    ...
    async function startBattle() {
        walkerRunning = false;
        SLOTS.forEach((slot) => clearWinAura(slot)); // 서버가 새로 계산한 stack_count로만 오라가 갱신되게, 수동으로 쌓아둔 건 초기화
        ...
        SLOTS.forEach((slot) => {
            ...
            units[slot] = {
                name: raw.name, maxHp: raw.max_hp, hp: raw.max_hp, isMelee: raw.is_melee, outfit: raw.outfit,
    ...
    function playEvents(events, index) {
        ...
        if (event.event_type === "trait_resolve") {
            ...
            if (actorSlot) {
                ...
                if (dispatchEffectType === "self_stack_buff" && event.detail?.stack_count) {
                    updateWinAura(actorSlot, event.detail.stack_count);
                }

                if (dispatchEffectType === "self_shield_duration" && event.detail?.shield_seconds) {
                    spawnShieldRing(actorSlot, event.detail.shield_seconds * 1000 * PLAYBACK_SPEED);
                }
            }
            ...
            if (dispatchEffectType === "conditional_target_debuff" && event.detail?.stunned && actorSlot) {
                ...
                if (hitSlot) {
                    playDualCrayonSkillProjectile(actorSlot, hitSlot, () => {
                        const targetUnitEl = document.querySelector(`[data-unit="${hitSlot}"]`);
                        targetUnitEl?.classList.add("skill-vfx-stun");
                        setTimeout(() => targetUnitEl?.classList.remove("skill-vfx-stun"), 450);
                    });
                }
                ...
                if (healSlot) {
                    spawnHealingHeart(healSlot, () => {
                        ...
                        renderUnit(healSlot);
                        const healImgEl = document.querySelector(`[data-unit="${healSlot}"] .battle-unit-img`);
                        healImgEl?.classList.add("heal-target-aura");
                        setTimeout(() => healImgEl?.classList.remove("heal-target-aura"), 900);
                    });
                }
    ...
    function resetAll() {
        ...
        document.querySelectorAll(".battle-unit").forEach((el) => {
            ...
            const imgEl = el.querySelector(".battle-unit-img");
            imgEl?.classList.remove("casting", "casting-rainbow", "walking", "attacking", "hit-flash", "crit-flash", "is-clone", "win-aura", "dying", "death-fallback-filter");
            imgEl?.style.removeProperty("--aura-color");
            imgEl?.style.removeProperty("--aura-blur");
        });
        ["attacker-summon", "defender-summon"].forEach((slot) => {
