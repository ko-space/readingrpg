...
(function () {
    ...
    function describeEffect(item) {
        ...
        if (item.effect_type === "super_success_shift") {
            return `강화 시 성공 확률 ${Math.abs(p.success_delta || 0)}%p를 슈퍼 성공(2성치 강화) 확률로 옮깁니다.`;
        }
        if (item.effect_type === "material_substitute") {
            return "강화 시 재료 인물 카드 1장을 대신합니다.";
        }
        if (item.effect_type === "next_enhance_buff") {
            return `강화 성공 시, 그 카드의 다음 강화에서 파괴 확률 ${Math.abs(p.destroy_delta || 0)}%p 감소, 성공 확률 ${p.success_delta || 0}%p 증가 효과를 예약합니다.`;
        }
        if (item.effect_type === "dust_convert") {
            // 성급별 확률표가 전부 100이면(귀환의 돌 등) "성급별 확률"이라는 표현이 오해를 준다 -
            // 실제로는 성급과 무관하게 항상 성공이라는 뜻이라 문구를 다르게 보여준다.
            const isGuaranteed = ["1", "2", "3", "4", "5"].every((star) => Number(p[star]) === 100);
            return isGuaranteed
                ? "강화 성공/유지/파괴 확률이 사라지고, 성급과 상관없이 인물 3장이 먼지 1개로 바뀝니다. 다른 강화 아이템과 함께 사용할 수 없습니다."
                : "강화 성공/유지/파괴 확률이 사라지고, 성급별 확률로 인물 3장이 먼지 1개로 바뀝니다. 다른 강화 아이템과 함께 사용할 수 없습니다.";
        }
        return "";
    }
