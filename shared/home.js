...
// 여기가 "창을 다시 켰을 때" 사실상 유일한 재진입점이다. reading.js가 세션을 저장/복구한다.
const __pendingReadingSession = window.ReadingSession ? window.ReadingSession.load() : null;
if (__pendingReadingSession) {
    window.location.href = window.ReadingSession.buildUrl(__pendingReadingSession);
}
...
const LOBBY_ENTERING_TIPS = [
    ...
    "투기장모드 티켓은 하루에 10개밖에 구매하지 못합니다.",
    "인물별로 지역 입장에서의 능력이 다릅니다.",
    "스토리모드 앤딩에는 히든 앤딩이 존재합니다.",
    "히든 업적 달성 보상을 얻기 위해 히든 업적을 달성해보세요.",
    "ester CAD라는 비밀 조직이 존재합니다.",
    "인물 3성 달성 시 [Passive] 스킬이 추가됩니다.",
    "인물 4성 달성 시 [Active] 스킬이 추가됩니다.",
    "인물 5성 달성 시 [Special] 스킬이 추가됩니다.",
    "6성 캐릭터는 무지막지하게 강력합니다.",
    "청년을 육성해 보세요.",
    "개발 초기 의미 있던 히든 업적은 3개였습니다.",
    "10레벨이 되는데 4500이상의 exp가 필요합니다.",
    "5성 -> 6성 강화를 아이템 없이 시도해볼까요?",
    "초기 영웅 등급 인물들은 아군을 죽일 수 있었습니다.",
    "상점에서 살 수 없는 아이템이 존재합니다.",
    "최대 레벨은 30레벨입니다.",
    "시간 측정은 밤 11시 59분이 지나면 종료됩니다."
];
...
function renderProfile(data) {
    ...
    silverValue.textContent = (user.silver || 0).toLocaleString();
