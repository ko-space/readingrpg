// 인연 스토리 Episode 1(윤대웅)의 대사/분기/캐릭터/배경 데이터. story/story-engine.js(엔진)가
// 이 파일을 전역으로 소비하므로, HTML에서 반드시 이 스크립트를 story-engine.js보다 먼저
// 로드해야 한다(story-relationship.html 참고). story/scenario/sub1_kimnamok.js와 같은 패턴.

// STORY_ID/AUTO_USE_STORAGE_KEY/ASSET_BASE/CHAR_IMG/BG/PLAYER/CG_GALLERY_ITEMS/
// TRUE_ENDING_REQUIREMENTS/TRUE_ENDING_GALLERY_IDS/ENDING_CG_ID_BY_TITLE는 const가 아니라 var다 -
// Episode 2가 로드되면(story/scenario/ep2_choijaehyeok.js) story-engine.js의 activateEpisodeBundle()이
// 이 값들을 그때그때 활성 에피소드의 것으로 바꿔치기하기 때문이다(파일 끝의 EP1_BUNDLE 참고). const로
// 두면 Episode 2가 같은 이름으로 자기 값을 준비했다가 전환 시점에 재할당하는 걸 애초에 막아버린다.
var STORY_ID = "ep1_yoondaewoong";
var AUTO_USE_STORAGE_KEY = "story_ep1_auto_use_tickets"; // 티켓 자동사용 여부는 서버 저장 대상이 아닌 브라우저별 UI 설정
var ASSET_BASE = "assets/story/ep1/";
...
var PLAYER = { name: '__PLAYER_NAME__', sub: '', key: null, hideSub: true };
...
// 실제로 해금된 CG까지 도감에서 잠긴 것처럼 보인다. BG가 아직 Episode 1 자신의 값인 지금(이 스크립트가
// 평가되는 시점) 실제 URL로 미리 구워서 imageSrcs로도 남겨두면, getGalleryImages가 그쪽을 먼저 쓰므로
// (ep2_choijaehyeok.js가 자기 항목에 imageSrcs를 쓰는 것과 같은 이유) 활성 에피소드와 무관해진다.
CG_GALLERY_ITEMS.forEach(item => {
  if(item.imageKeys){
    item.imageSrcs = item.imageKeys.map(key => BG[key]).filter(Boolean);
  }
});
/* HIDDEN ENDING은 TRUE ENDING 해금 조건에 포함하지 않는다. */
var TRUE_ENDING_REQUIREMENTS = [
  'bad',
  'normal',
  'juheon',
  'seungyu',
...
var TRUE_ENDING_GALLERY_IDS = [
  'true_seungyu',
  'true_ganghee',
  'true_yeongwoong',
  'true_juheon',
];
var ENDING_CG_ID_BY_TITLE = {
  'BAD END': 'bad',
  'BAD ENDING': 'bad',
  '평범한 일상 END': 'normal',
  'NORMAL END': 'normal',
...
// Episode 2가 활성화되면(activateEpisodeBundle) 위 STORY_ID/CHAR_IMG/BG/... 값들이 전부 Episode 2의
// 것으로 바뀌어버리므로, "Episode 1로 되돌아가기"가 가능하도록 지금(=Episode 1 값 그대로인 시점) 스냅샷을
// 남겨둔다. GALLERY_EPISODE_SECTIONS(도감)는 반대로 활성 에피소드와 무관하게 항상 모든 에피소드를
// 동시에 보여줘야 해서 여기서 정의하지 않는다 - 두 에피소드 데이터가 전부 로드된 뒤인
// story/scenario/ep2_choijaehyeok.js 쪽에서 EP1_BUNDLE.CG_GALLERY_ITEMS를 참조해 함께 구성한다.
var EP1_BUNDLE = {
  STORY_ID, AUTO_USE_STORAGE_KEY, ASSET_BASE, CHAR_IMG, BG, PLAYER,
  CG_GALLERY_ITEMS, TRUE_ENDING_REQUIREMENTS, TRUE_ENDING_GALLERY_IDS, ENDING_CG_ID_BY_TITLE,
};
