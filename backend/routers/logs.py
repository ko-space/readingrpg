...
HEARTBEAT_MAX_CREDIT_SECONDS = 120  # 확인 한 번당 인정하는 최대 시간 - 핵심 방어선. 정상 주기(15초)의
# 8배까지 넉넉하게 잡아서, 와이파이 끊김/이동통신 기지국 전환/서버 재배포처럼 실제로 흔히 생기는
# 네트워크 장애(보통 수 초~길어도 1분 이내로 복구됨)는 사실상 전부 이 상한 안에서 그대로 흡수돼
# 화면 표시 시간과 실제 인정 시간이 벌어지는 일이 없다(확인된 문제 - 45초로도 여전히 눈에 띄게
# 벌어지는 경우가 있었다). 프론트(reading.js의 heartbeat 재시도 로직)도 장애 중엔 훨씬 짧은 주기로
# 계속 재시도해서, 연결이 복구되는 즉시 하트비트가 성공하도록 되어 있다 - 즉 이 상한에 실제로 걸리려면
# "몇 분 이상 이어지는" 드문 장애여야 한다. 그런 드문 경우에도, 탭을 통째로 방치하거나(수십 분~시간
# 단위) 기기 시간을 조작하는 것과 비교하면 여전히 압도적으로 작은 상한이라 방어 효과 자체는 유지된다 -
# 이 상한을 없애버리면(무제한 인정) 탭을 열어만 두고 네트워크 요청만 의도적으로 차단한 채 방치했다가
# 나중에 한 번에 몰아서 인정받는 것을 막을 방법이 없어진다.
