...
def _designer_user_id(name: str) -> int:
    catalog = CATALOG_BY_NAME.get(name)
    if catalog and catalog.get("designer_user_id") is not None:
        return catalog["designer_user_id"]
    return DEFAULT_DESIGNER_USER_ID


def _attach_designer_nicknames(db: Session, entries: list[dict]):
    needed_ids = {_designer_user_id(e["name"]) for e in entries}
    nickname_by_id = dict(db.query(User.id, User.nickname).filter(User.id.in_(needed_ids)).all())
    for entry in entries:
        entry["designer_nickname"] = nickname_by_id.get(_designer_user_id(entry["name"]))
