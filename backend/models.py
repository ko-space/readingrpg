...
class Character(Base):
    ...
    is_indestructible = Column(Boolean, default=False)  # 특정 히든 업적 보상 캐릭터 전용 - 강화 파괴 판정이 유지로 바뀜
...
class ReadingSessionState(Base):
    ...
    재서 그대로(상한 없이) 누적한다 - 클라이언트가 무엇을 보내든(또는 아무것도 안 보내든) 영향을
    받지 않는다. 탭을 다른 앱 뒤로 보내두고 몇 시간이 지나도(다른 곳에서 실제로 읽고 있는 경우 등)
    일시정지만 안 눌렀다면 그 시간 전부가 정상적으로 인정되도록 의도된 설계다(확인된 요청) - 그래서
    한 번 확인당 상한을 두지 않는다. 라우터(routers/logs.py)의 _flush_session_seconds 참고."""
