...
(function () {
    ...
    // 간격 자체는 보상 계산과 무관(상한이 없어져서 "자주 확인해야 안 깎인다"는 이유가 사라짐) - 화면을
    // 서버 값에 얼마나 자주 맞춰 보여주느냐의 문제일 뿐이라, DB 호출 횟수(Supabase egress) 절감을 위해
    // 넉넉하게 60초로 잡는다. 초 단위 화면 표시는 로컬 타이머(getElapsedMs)가 매초 갱신하므로 하트비트
    // 간격을 늘려도 화면이 끊겨 보이지 않는다 - 그저 서버 확정치와의 재동기화가 조금 덜 자주 일어날 뿐.
    const HEARTBEAT_INTERVAL_MS = 60000;
    const HEARTBEAT_RETRY_MS = 5000; // 실패 시 재시도 간격 - 정기 주기를 기다리지 않고 훨씬 짧게 계속
